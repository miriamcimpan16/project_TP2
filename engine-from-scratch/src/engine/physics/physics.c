...
void physics_update(void) {
	...
	for (u32 i = 0; i < state.body_list->len; ++i) {
        ...
        f32 velocity_length = vec2_len(scaled_velocity);
		u32 dynamic_iterations = 1 + (u32)(velocity_length / 50.0f); // 50 e arbitrar, ajustezi
		if (dynamic_iterations > 20) dynamic_iterations = 20; // limităm

		vec2 per_iteration_velocity;
		vec2_scale(per_iteration_velocity, scaled_velocity, 1.f / dynamic_iterations);
